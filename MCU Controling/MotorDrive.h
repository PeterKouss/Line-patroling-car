...
#define INITIAL_PWM_CYCLE (50) //zhan kong bi

#define PWM_MIN_SET (15)
...
void Init_GPIO(void) //GPIO For Motor Drive
{
...
void Init_MotorDrive()
{
	__enable_irq();
	Init_PMW();
	Init_GPIO();
	Init_read_PMW();
	while (TPM1->CONTROLS[0].CnV != INITIAL_PWM_CYCLE * PWM_MAX_COUNT / 100)
	{
		Set_PWM_Value(INITIAL_PWM_CYCLE, 0);
	}
	while (TPM1->CONTROLS[1].CnV != INITIAL_PWM_CYCLE * PWM_MAX_COUNT / 100)
	{
		Set_PWM_Value(INITIAL_PWM_CYCLE, 1);
	}
}

void Set_MotorSpeed(float speed_set_1, float speed_set_2) //set_1 left£¬set_2 right
{
	int pwm_control_1;
	int pwm_control_2;
	int pwm_set_1;
	...
	{
		pwm_control_1 = (k_1 * speed_set_1) + (kp_1 * (speed_set_1 - Speed_Caculate(1)));
		if (pwm_control_1 > 99)
		{
			pwm_set_1 = 99;
		}
		else if (pwm_control_1 < PWM_MIN_SET)
		{
			pwm_set_1 = PWM_MIN_SET;
		}
		else
		{
			pwm_set_1 = pwm_control_1;
		}
	...
	{
		pwm_control_2 = (k_2 * speed_set_2) + (kp_2 * (speed_set_2 - Speed_Caculate(2)));
		if (pwm_control_2 > 99)
		{
			pwm_set_2 = 99;
		}
		else if (pwm_control_2 < PWM_MIN_SET)
		{
			pwm_set_2 = PWM_MIN_SET;
		}
		else
		{
			pwm_set_2 = pwm_control_2;
		}
